...
/**
 * JCCDev Component Model
 *
 * @package     JCCDev
 * @subpackage  Models
 */
class JCCDevModelComponent extends JModelAdmin
{
	/**
	...
	public function delete(&$pks)
	{
		...
		if ( (int) JComponentHelper::getParams('com_jccdev')->get('delete_tables') );
		{
			...
			foreach ($pks as $pk)
			{
				...
				$db->setQuery($query->select('id')->from('#__jccdev_tables')->where('`component` = ' . $db->quote((int) $pk)));
				$ids = $db->loadRowList();
				$keys = array();
	 ...
	 * @throws  Exception
	 */
	public function getTable($type = 'Component', $prefix = 'JCCDevTable', $config = array())
	{
	...
	{
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_jccdev.edit.component.data', array());
