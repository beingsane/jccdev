...
/**
 * JCCDev Plugins Model
 *
 * @package     JCCDev
 * @subpackage  Models
 */
class JCCDevModelPlugins extends JModelList
{
	/**
	 * Constructor
	 *
	...
	protected function populateState($ordering = 'name', $direction = 'ASC')
	{
		...
		$authorId = $app->getUserStateFromRequest($this->context . '.filter.folder', 'filter_folder');
		$this->setState('filter.folder', $authorId);
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->createDir = JCCDevArchive::getArchiveDir() . "/" . JCCDevArchive::getArchiveName("plg_", $item->name, $item->version);
			$item->installed = JCCDevInstall::isInstalled("plugin", $item->name, $item->folder);
			
			if ($item->installed)
			{
				$extension->load(array("name" => "plg_" . $item->folder . "_" . $item->name, "type" => "plugin", "folder" => $item->folder));
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__jccdev_plugins AS a');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__jccdev_plugins AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
