...
class JFormFieldFormParent extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		$query = $db->getQuery(true)
			...
			->from('#__jccdev_forms AS a')
			->join('LEFT', $db->quoteName('#__jccdev_forms') . ' AS b ON a.lft > b.lft AND a.rgt < b.rgt')
			->where('a.parent_id > 0');

		// Prevent parenting to children of this item.
		if ($id = $this->form->getValue('id'))
		{
			$query->join('LEFT', $db->quoteName('#__jccdev_forms') . ' AS p ON p.id = ' . (int) $id)
