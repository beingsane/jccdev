...
/**
 * JCCDev Template Model
 *
 * @package     JCCDev
 * @subpackage  Models
 */
class JCCDevModelTemplate extends JModelAdmin
{
	/**
	...
	{
		$model_pl = JModelLegacy::getInstance("Overrides", "JCCDevModel");
		$model_sg = JModelLegacy::getInstance("Override", "JCCDevModel");

		// Delete overrides
		foreach ($pks as $pk)
		{
			$overrides = $model_pl->getOverrides("template", $pk);
			if (count($overrides))
			{
	 ...
	 * @throws  Exception
	 */
	public function getTable($type = 'Template', $prefix = 'JCCDevTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$form = $this->loadForm('templates', 'template', $options);
	...
	public function getItem($pk = null)
	{
		...
		$item->installed = JCCDevInstall::isInstalled("template", "tpl_" . $item->name);
		$item->createDir = JCCDevArchive::getArchiveDir() . "/" . JCCDevArchive::getArchiveName("tpl_", $item->name, $item->version);
	...
	{
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_jccdev.edit.template.data', array());
