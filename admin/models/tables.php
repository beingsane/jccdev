 ...
 * @subpackage  Models
 */
class JCCDevModelTables extends JModelList
{
	/**
	 * Component tables
	 *
	...
	{		
		...
		foreach ($items as $item)
		{
			...
			$db->setQuery("SELECT COUNT(*) FROM `#__jccdev_fields` AS f WHERE f.table = " . $item->id);
			$item->numberOfFields = $db->loadResult();
			
			$db->setQuery("SELECT f.name FROM #__jccdev_fields AS f WHERE f.table = ".$item->id." ORDER BY f.ordering");
			$item->mainfield = $db->loadResult();
			
			$formfile = JCCDARCHIVE . "/tables/table_" . $item->id . ".xml";
			$item->formfile = (JFile::exists($formfile)) ? JCCDARCHIVEURL . "/tables/table_" . $item->id . ".xml" : "";
	...
	public function getComponentTables($com_id)
	{
		if (!isset(self::$tables[$com_id]))
		{
			...
			$query = $db->getQuery(true)->select("a.id, a.created_by, a.component")->from("#__jccdev_tables AS a")->where("a.component = " . $db->quote($com_id));
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__jccdev_tables AS a');
		
		//Join over components
		$query->select('c.display_name AS ' . $db->quote('component'))
			->join('LEFT', '#__jccdev_components AS c ON c.id = a.component');
