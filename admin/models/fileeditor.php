 ...
 * @subpackage  Models
 */
class JCCDevModelFileeditor extends JModelForm
{
	/**
	...
	protected function loadFormData()
	{
		...
		$data = $app->getUserState('com_jccdev.edit.fileeditor.data', array());
	...
	public function getItem($pk = null)
	{
		...
		switch ($type) {
			case "component" :
				if ($client == "admin")
					$filepath = JCCDevPath::dots2ds(JPATH_ADMINISTRATOR . "/components/com_" . $extName . "/" . $path);
				else
					$filepath = JCCDevPath::dots2ds(JPATH_SITE . "/components/com_" . $extName . "/" . $path);

				if ($filepath === false)
					throw new Exception(JText::_("COM_JCCDEV_FILEMANAGER_FILE_NOT_FOUND"));
