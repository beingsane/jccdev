 ...
 * @subpackage  Models
 */
class JCCDevModelFields extends JModelList
{
	/**
	 * Table fields
	 *
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$db->setQuery("SELECT c.name AS 'component' FROM `#__jccdev_components` AS c WHERE c.id = " . $item->table_id);
	...
	public function getTableFields($table_id)
	{
		if (!isset(self::$fields[$table_id]))
		{
			...
			$query = $db->getQuery(true)->select("a.id, a.table, a.name")->from("#__jccdev_fields as a")->where("a.table = " . $db->quote($table_id))->order("a.ordering ASC");
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from($db->quoteName('#__jccdev_fields') . 'AS a');
		
		//Join over tables
		$query->select('t.name AS '. $db->quote('table'))
			->select('t.id AS ' . $db->quote('table_id'))
			->join('LEFT', '#__jccdev_tables AS t ON t.id = a.table');

		// Join over the jccdev formfields
		$query->select('ff.id AS jccdev_formfield_id')
			->join('LEFT', '#__jccdev_formfields AS ff ON ff.name = a.type');

		// Join over the jccdev formrules
		$query->select('fr.id AS jccdev_formrule_id')
			->join('LEFT', '#__jccdev_formrules AS fr ON fr.name = a.rule');
