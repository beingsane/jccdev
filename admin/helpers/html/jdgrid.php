...
class JHtmlJDGrid
{
	public static function archives($prefix, $name)
	{
		...
		if (count(JCCDevArchive::getVersions($prefix, $name)))
		{
			foreach (JCCDevArchive::getVersions($prefix, $name) as $file)
			{
				$download = JCCDevArchive::getArchiveDir(true) . "/$file";
				$html .= '<a href="'.$download.'" class="hasTooltip" title="' . JText::_('COM_JCCDEV_COMPONENT_DOWNLOAD') . '">'.$file.'</a> - <span style="color:#999999;">'.date("Y.m.d H:i", filemtime(JCCDevArchive::getArchiveDir() . "/$file")).'</span><br>';
			}
		}
		else
		{
