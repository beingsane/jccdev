 ...
 * @subpackage  Create.Table
 */
class JCCDevCreateTable extends JCCDevCreate
{	
	/*
	 * The component data
	 *
	...
	public function __construct($config = array())
	{
		...
		if (!isset($config['item_id']) || empty($config['item_id'])) throw new JCCDevException($this->_name . ": No table id given");
	...
	public static function execute($client, $config = array())
	{
		...
		foreach (JFolder::files(JCCDevCREATE . "/table/$client", "php$") as $file)
		{
			// Execute create classes for each table
			foreach (JModelLegacy::getInstance("Tables", "JCCDevModel")->getComponentTables($component->id) as $table)
			{
				$table_config = array("item_id" => $table->id);
				$class = JCCDevCreate::getInstance("table.$client." . JFile::stripExt($file), $table_config);
	...
	protected function loadOverride($type = "", $item_id = "", $name = "")
	{
		...
		{
			$type = "table.";
			$type .= preg_match("/^JCCDevCreateTableAdmin/", $this->_name) ? "admin" : "site";
		}
