 ...
 * @subpackage  Create.Template
 */
class JCCDevCreateTemplate extends JCCDevCreate
{	
	/*
	...
	{		
		...
		$this->templateDirs[0] = JCCDevXTD . "/templates/template";
		$this->templateDirs[1] = JCCDevTEMPLATES . "/template";
	...
	{
		$item = JModelLegacy::getInstance("Template", "JCCDevModel")->getItem($config['item_id']);
		$dir = $item->createDir;
		...
		// Copy files
		JFile::copy(JCCDevTEMPLATES . "/template/template.css", $dir . "/css/template.css");
		
		foreach (JFolder::files(JCCDevCREATE . "/template", "php$") as $file)
		{
			$class = JCCDevCreate::getInstance("template." . JFile::stripExt($file), $config);
			if (!$class->create())
			{
		...
		JCCDevCreate::getInstance("language.template", $config)->create();
		JCCDevCreate::getInstance("language.template.sys", $config)->create();

		JFile::copy(JCCDevTEMPLATES . "/template/favicon.ico", $dir . "/favicon.ico");
	}
	...
	{
		return JCCDevLanguage::getStaticInstance("tpl_" . $this->item->name);
	}
