 ...
 * @subpackage  Create.Table
 */
class JCCDevCreateTableAdminTable extends JCCDevCreateTable
{		
	/**
	 * The template file
	 *
	...
	protected function initialize()
	{
		...
		$this->getLanguage()->addKeys(array(
			...
			'FIELD_' . $this->table->pk . '_ORDERING_ASC'	=> JText::sprintf('COM_JCCDEV_ORDER_ASC', ucfirst($this->table->pk)),
			'FIELD_' . $this->table->pk . '_ORDERING_DESC'	=> JText::sprintf('COM_JCCDEV_ORDER_DESC', ucfirst($this->table->pk))
		), $this->table->name);
		...
		foreach ($this->fields as $field)
		{
			...
			$this->getLanguage()->addKeys(array(
				...
				"ORDERING_ASC"	=> JText::sprintf('COM_JCCDEV_ORDER_ASC', ucfirst($field->name)),
				"ORDERING_DESC"	=> JText::sprintf('COM_JCCDEV_ORDER_DESC', ucfirst($field->name)),
				"KEEP"			=> JText::sprintf('COM_JCCDEV_KEEP_VALUE', ucfirst($field->name)),
		...
		if (isset($this->table->params["relations"]))
		{
			foreach ($this->table->params["relations"] as $relation)
			{
				...
				$this->getLanguage()->addKeys(array(
					...
					"ORDERING_ASC"	=> JText::sprintf('COM_JCCDEV_ORDER_ASC', ucfirst($table->singular)),
					"ORDERING_DESC"	=> JText::sprintf('COM_JCCDEV_ORDER_DESC', ucfirst($table->singular)),
					"KEEP"			=> JText::sprintf('COM_JCCDEV_KEEP_VALUE', ucfirst($table->singular)),
