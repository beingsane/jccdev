...
class JCCDevArchive
{
	...
	{
		$PATH_ADMIN = JCCDevLIVE . "/" . $extension . "/" . $archivelangdir;
	...
	public static function deleteOldVersions($prefix, $name, $version = "1.0.0")
	{
		...
		{			
			...
			{
				if (!JFile::delete(JCCDARCHIVE.DS . $file)) throw new JCCDevException("Wrong path - $file");
			}
		}
	}
	 ...
	 public static function getArchiveDir($url = false)
	{
		...
		$dir = $params->get("userarchives", 0) ? JCCDARCHIVE . "/user_" . $user->id : JCCDARCHIVE;
		if ($url) $dir = $params->get("userarchives", 0) ? JCCDARCHIVEURL . "/user_" . $user->id : JCCDARCHIVEURL;
		return $dir;
	}
...
//	print $name;die();
	...
	{
		// Get stream
		if (empty($folder)) throw new JCCDevException('No folder given');
		if (!JFolder::exists($folder)) JFolder::create($folder);
		$handle = opendir($folder);
		
		// Write an html file
		if (!$file = fopen($folder .DS. "index.html","w")) throw new JCCDevException("Couldn`t open file $folder/index.html");
	...
	{
		$path = JCCDARCHIVE.DS . $prefix . strtolower($name);
		$path .= '-' . $version;
	...
	public static function lastZip($name, $version = "1.0", $prefix = 'com_')
	{
		...
		$path = JCCDARCHIVE.DS . $prefix . strtolower($name) . '-' . $version;		
