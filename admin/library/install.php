...
class JCCDevInstall
{
	...
	{
		$folders = JFolder::folders(JCCDevINSTALL);
		$files = JFolder::files(JCCDevINSTALL);
		
		foreach ($folders as $folder) JFolder::delete(JCCDevINSTALL.DS.$folder);
		foreach ($files as $file) if ($file != 'index.html') JFile::delete(JCCDevINSTALL.DS.$file);
	}
	...
	public static function install($file, $update = false)
	{
		...
		$tmp = JCCDevINSTALL . "/" . JFile::stripExt( JFile::getName($file) );
