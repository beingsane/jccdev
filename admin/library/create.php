...
{	
	...
	/**
	 * Get an JCCDevCreate object
	 *
	 * @param	string	$name	The class name
	 * @param	array	$config	The configuration data
	 *
	 * @return	mixed	instance of JCCDevCreate on success, false otherwise
	 */
	public static function getInstance($name, $config = array())
	{
		$class = "JCCDevCreate" . JCCDevLoader::getClassname($name);
		
		if (!class_exists($class))
		{
			JCCDevLoader::import($name, JCCDevCREATE);
		}
		...
		{
			throw new JCCDevException("Could not find class <i>$class</i>");
		}
	}
	 ...
	 */
	protected function getModel($name, $prefix = 'JCCDevModel')
	{
	...
	protected function getTemplate()
	{
		...
		{
			return new JCCDevTemplate($override->source, false);
		}
		
		if ($this->template instanceof JCCDevTemplate)
		{
		...
		{			
			...
			{
				$path = JCCDevPath::dots2ds($templateDir . "/" . $this->templateFile);

				if ($path !== false)
				{
					return new JCCDevTemplate($path);
				}
			}
	 ...
	 *
	 * @return	JCCDevTemplate		The subtemplate object
	 */
	protected function loadOverride($type = "", $item_id = "", $name = "")
	{
		return JModelLegacy::getInstance("Override", "JCCDevModel")->getOverride($type, $item_id, $name);
	}
	 ...
	 *
	 * @param   JCCDevTemplate  $template    The template to add.
	 *
	 * @return  object   The template.
	 */
	protected function setTemplate($template)
	{
		if (!$template instanceof JCCDevTemplate)
		{
			throw new JCCDevException($this->name . ": Template must be instance of JCCDevTemplate");
		}
