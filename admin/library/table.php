...
class JCCDevTable
{
	...
	public static function getDbTableFields($table, $ignore)
	{
		...
		foreach ($dbfields as $dbfield)
		{
			...
			// Set params
			$form = JModelLegacy::getInstance("Field", "JCCDevModel")->getForm();
	...
	{
		require_once JCCDevLIB.DS.'template.php';
		
		$template = new JCCDevTemplate(JCCDevPath::dots2ds(JCCDevTEMPLATES.DS.'com.admin.sql.install.mysql.utf8.create.sql'));
		$table = JModelLegacy::getInstance('Table', 'JCCDevModel')->getItem($id);
		$component = JModelLegacy::getInstance('Table', 'JCCDevModel')->getItem($table->component);
		$fields = JModelLegacy::getInstance('Fields', 'JCCDevModel')->getTableFields($table->id);
		...
		$template->addPlaceholders(array(
			'table_db'		=> (JComponentHelper::getParams('com_jccdev')->get('add_component_name_to_table_name')) ? $component->name . '_' . $table->name : $table->name,
			'pk'			=> $table->pk,
