...
<form action="<?php JRoute::_('index.php?option=com_jccdev&id=' . (int)$this->item->id); ?>" method="post" name="adminForm" id="form-form" class="form-validate">

	<?php if ($input->get("id", 0) == 0 && $input->get("parent_id", 0) < 2) : ?>
		<h2><?php echo JText::_("COM_JCCDEV_FORM_NEW_FORM"); ?></h2>
	<?php elseif ($input->get("id", 0) != 0 && $input->get("parent_id", 0) < 2) : ?>
		<h2><?php echo JText::_("COM_JCCDEV_FORM_EDIT_FORM"); ?></h2>
	<?php elseif ($input->get("id", 0) == 0 && $input->get("parent_id", 0) > 1) : ?>
		<h2><?php echo JText::_("COM_JCCDEV_FORM_NEW_FIELD"); ?></h2>
	<?php elseif ($input->get("id", 0) != 0 && $input->get("parent_id", 0) > 1) : ?>
		<h2><?php echo JText::_("COM_JCCDEV_FORM_EDIT_FIELD"); ?></h2>
	<?php endif; ?>
