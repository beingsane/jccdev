...
<?php foreach ($this->items as $i => $item) : ?>
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td>
			<a href="<?php echo JRoute::_('index.php?option=com_jccdev&task=table.edit&id=' . $item->id); ?>">#__<?php echo $this->escape($item->name); ?></a>
			<br>
			<i style="color:grey; font-size:12px"><?php echo $this->escape($item->component); ?></i>
		</td>
		<td><?php echo JHtml::_("jdgrid.assigned", JRoute::_('index.php?option=com_jccdev&view=fields&filter[table]=' . $item->id), $this->escape($item->numberOfFields)); ?></td>
		<td>
